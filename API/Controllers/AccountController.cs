...
namespace API.Controllers
{
    public class AccountController : BaseApiController
    {
        ...
        [HttpPost("login")]
        //takes a login dto and returns a user dto
        public async Task<ActionResult<UserDto>> Login(LoginDto loginDto)
        {
            var user = await _userManager.FindByEmailAsync(loginDto.Email);
            //if no user matches the login Dto then return unauthorized
            if (user == null) return Unauthorized(new ApiResponse(401));
            //check whether the user successfully signed in or not
            var result = await _signInManager.CheckPasswordSignInAsync(user, loginDto.Password, false);
            //if the request didnt succeed return unauthorized
            if(!result.Succeeded) return Unauthorized(new ApiResponse(401));
            //if the result succeeded return the logged in user dto
            return new UserDto
            {
                Email = user.Email,
                Token = _tokenService.CreateToken(user),
